...
app.layout = html.Div([
    ...
    ], style={'display': 'flex', 'justifyContent': 'space-around', 'padding': '20px', 'backgroundColor': '#f9f9f9'}),
...
def store_dashboard(Region, Category, Sub_Category, Segment, start_date, end_date, Ship_Mode):
    ...
    bar_fig.update_traces(marker_line_width=0, selector=dict(type="bar"))
    ...
    abar_fig.update_traces(marker_line_width=0)
    abar_fig.update_layout(showlegend=False, plot_bgcolor='white')

    line_fig = px.line(df.groupby('Order Date', as_index=False)['Sales'].sum(),
                       x='Order Date', y='Sales', markers=True, title="Sales Over Time")
...
if __name__ == '__main__':
    port = int(os.environ.get('PORT', 8050))  # Render will set this automatically
